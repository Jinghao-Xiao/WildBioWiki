...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate image-level captions for WildBioWiki-QA-medium with LLaVA-OneVision-1.5-8B-Base."
    )
    parser.add_argument("--manifest", default="data/WildBioWiki-QA-medium/manifests/val.jsonl")
    parser.add_argument("--model-dir", default="models/LLaVA-OneVision-1.5-8B-Base")
    parser.add_argument(
        "--output-jsonl",
        default="data/WildBioWiki-QA-medium/captions/llava_onevision_1_5_8b_base/val.jsonl",
    )
...
def load_completed_ids(output_jsonl: Path) -> set:
    if not output_jsonl.exists():
        return set()
    return {str(row["image_id"]) for row in load_jsonl(output_jsonl)}
