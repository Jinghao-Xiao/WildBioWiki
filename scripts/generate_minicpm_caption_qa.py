...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate caption and QA answers with MiniCPM-V-4_5."
    )
...
def load_minicpm_tokenizer(model_dir: Path):
    tokenizer_module_path = model_dir / "tokenization_minicpmv_fast.py"
    spec = importlib.util.spec_from_file_location(
        "minicpm_tokenization_minicpmv_fast", tokenizer_module_path
    )
    module = importlib.util.module_from_spec(spec)
    assert spec is not None and spec.loader is not None
    spec.loader.exec_module(module)
    tokenizer_cls = module.MiniCPMVTokenizerFast
...
def main() -> int:
    ...
    tokenizer = load_minicpm_tokenizer(model_dir)
